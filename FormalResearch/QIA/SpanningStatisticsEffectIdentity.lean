...
first known only on the reduced coherent-copy family.  Once that family spans
the full real Hermitian multiplicity algebra, equality of all trace statistics
on the family forces equality of the corresponding Hermitian effects.

This module formalizes exactly that finite linear-algebra extension.  It does
not prove that the physical coherent-copy family spans; that remains the
source-specific coherent-power polarization/separation bridge.
-/
...
theorem multiplicityHermitian_eq_of_trace_eq_on_spanning_family
  ...
  have hf_generator : ∀ X ∈ s, f X = 0 := by
    intro X hX
    simpa [f, sub_eq_zero] using hstats X hX
...
/-- Separation-specialized version: if the chosen family separates Hermitian
directions under the summed trace pairing, then equality of trace statistics
on that family forces equality of Hermitian effects. -/
